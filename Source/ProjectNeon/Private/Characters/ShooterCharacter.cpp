...
void AShooterCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = CastChecked<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		EnhancedInputComponent->BindAction(FireAction, ETriggerEvent::Triggered, this, &AShooterCharacter::FireButtonPressed);
...
void AShooterCharacter::Look(const FInputActionValue& Value)
{
	...
	AddControllerYawInput(TurnScaleFactor * LookAxisVector.X);
}
...
void AShooterCharacter::CalculateCrosshairSpread(float DeltaTime)
{
	...
	FVector Velocity = FVector(GetVelocity());
	Velocity.Z = 0.f;
...
bool AShooterCharacter::TraceUnderCrosshairs(FHitResult& OutHitResult, FVector& OutHitLocation)
{
	...
	bool bScreenToWorld = UGameplayStatics::DeprojectScreenToWorld(UGameplayStatics::GetPlayerController(this, 0), CrosshairLocation, CrosshairWorldPosition, CrosshairWoldDirection);

	if (bScreenToWorld)
	{
		const FVector Start = CrosshairWorldPosition;
		const FVector End = Start + CrosshairWoldDirection * 50000.f;
		OutHitLocation = End;
...
void AShooterCharacter::PlayReloadMontage()
{
	...
	// TODO: switch on EquippedName->WeaponType
	UAnimInstance* AnimInstance = GetMesh()->GetAnimInstance();
